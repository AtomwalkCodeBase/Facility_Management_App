...
import { View, TouchableOpacity, StyleSheet, Keyboard, StatusBar, SafeAreaView,Platform,ScrollView,Dimensions,Image,Text } from "react-native";
...
const LoginScreen = () => {
  ...
  const fetchDbName = async () => {
    try {
      ...
      const prevDBName =
        (await AsyncStorage.getItem("previousDbName")) || savedDBName;
    ...
    try {
      ...
      if (selectedCompany) {
        const selected = dbList.find(
          (c) => c.ref_cust_name === selectedCompany.value
        );
        if (selected) {
          const newDbName = selected.name.replace(/^SD_/, "");
          await AsyncStorage.multiSet([
